...
int main(void) {
    ...
    char* message_template = "Das Fenster hat %3d Zeilen und %3d Spalten";
    int msg_len;
    int min_rows = 3;
    ...
    if ( LINES < min_rows || COLS < msg_len ) {
        ...
        mvaddch(0, 0, 'A');          // Move to position
                                    // Put character there XXXXXXXX
        ...
        for(int col = 1; col < COLS - 1; col++) {
            mvaddch(LINES - 1, col, 'F');
        }
 
        // Write our message centered onto the display
        mvprintw(mid_row, start_col, message_template, LINES, COLS);
